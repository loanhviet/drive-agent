...
def test_ui_has_documents_panel_and_safe_citation_links():
    source = Path("static/index.html").read_text(encoding="utf-8")

    assert 'id="documentsPanel"' in source
    assert "/api/drive/sync" in source
    assert "/api/drive/documents" in source
    assert "safeDriveLink" in source
    assert "drive.google.com" in source
    assert "citation.snippet" in source
